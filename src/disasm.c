...
int disasm(uint16_t pc, uint8_t bank, uint8_t *RAM, char *line, unsigned int max_line, int16_t x16Bank, uint8_t implied_status, int32_t *eff_addr) {
	uint8_t opcode = debug_read6502(pc, bank, x16Bank);
	...
	*eff_addr = -1;
	...
	if (isBlockMove) {
		snprintf(line, max_line, mnemonic, debug_read6502(pc + 2, bank, x16Bank), debug_read6502(pc + 1, bank, x16Bank));
		length = 3;
		if (regs.c != 0xFFFF) *eff_addr = regs.y; // We can have only one effective address, so we're choosing the destination
	}
	else if (isZprel) {
		snprintf(line, max_line, mnemonic, debug_read6502(pc + 1, bank, x16Bank), pc + 3 + (int8_t)debug_read6502(pc + 2, bank, x16Bank));
		length = 3;
	}
	else if (isRel16) {
		snprintf(line, max_line, mnemonic, (pc + 3 + (int16_t)(debug_read6502(pc + 1, bank, x16Bank) | (debug_read6502(pc + 2, bank, x16Bank) << 8))) & 0xffff);
		length = 3;
	}
	else if (isBrkOrCop) {
		snprintf(line, max_line, mnemonic, debug_read6502(pc + 1, bank, x16Bank));
		length = 2;
	} else {
		if (strstr(line, "%02x")) {
			length = 2;
			if (isRel8) {
				snprintf(line, max_line, mnemonic, pc + 2 + (int8_t)debug_read6502(pc + 1, bank, x16Bank));
			} else {
				snprintf(line, max_line, mnemonic, debug_read6502(pc + 1, bank, x16Bank));
				if (isStackRel) {
					uint16_t ptr = regs.sp + debug_read6502(pc + 1, bank, x16Bank);
					uint8_t ind_bank = ptr < 0xc000 ? memory_get_ram_bank() : memory_get_rom_bank();
					if (isIndirect && isYrel) {
						*eff_addr = (debug_read6502(ptr, bank, ind_bank) | (debug_read6502(ptr + 1, bank, ind_bank) << 8)) + regs.y;
					} else {
						*eff_addr = ptr;
					}
				} else if (isIndirect) {
					uint16_t ptr = debug_read6502(pc + 1, bank, x16Bank);
					if (isXrel)
						ptr += regs.x;
					*eff_addr = debug_read6502(ptr, bank, x16Bank) | (debug_read6502(ptr + 1, bank, x16Bank) << 8);
					if (isYrel)
						*eff_addr += regs.y;
				} else if (!isImmediate) {
					*eff_addr = debug_read6502(pc + 1, bank, x16Bank);
					if (isXrel)
						*eff_addr += regs.x;
					if (isYrel)
						*eff_addr += regs.y;
		...
		if (strstr(line, "%04x")) {
			length = 3;
			snprintf(line, max_line, mnemonic, debug_read6502(pc + 1, bank, x16Bank) | debug_read6502(pc + 2, bank, x16Bank) << 8);
			if (isIndirect) {
				uint16_t ptr = debug_read6502(pc + 1, bank, x16Bank) | (debug_read6502(pc + 2, bank, x16Bank) << 8);
				if (isXrel)
					ptr += regs.x;
				uint8_t ind_bank = ptr < 0xc000 ? memory_get_ram_bank() : memory_get_rom_bank();
				*eff_addr = debug_read6502(ptr, bank, ind_bank) | (debug_read6502(ptr + 1, bank, ind_bank) << 8);
				if (isYrel)
					*eff_addr += regs.y;
			} else if (!isImmediate) {
				*eff_addr = debug_read6502(pc + 1, bank, x16Bank) | (debug_read6502(pc + 2, bank, x16Bank) << 8);
				if (isXrel)
					*eff_addr += regs.x;
				if (isYrel)
					*eff_addr += regs.y;
		...
		if (strstr(line, "%06x")) {
			length = 4;
			snprintf(line, max_line, mnemonic, debug_read6502(pc + 1, bank, x16Bank) | debug_read6502(pc + 2, bank, x16Bank) << 8 | debug_read6502(pc + 3, bank, x16Bank) << 16);
			if (isIndirect) {
				uint16_t ptr = debug_read6502(pc + 1, bank, x16Bank) | (debug_read6502(pc + 2, bank, x16Bank) << 8);
				if (isXrel)
					ptr += regs.x;
